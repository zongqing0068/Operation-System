...
found:
  ...
  // 将kstack到kstack_pa的映射保存到内核独立页表中
  kvmmap_new(p->k_pagetable, (uint64)p->kstack, p->kstack_pa, PGSIZE, PTE_R | PTE_W);
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        w_satp(MAKE_SATP(p->k_pagetable));  // 切换每个进程的内核独立页表，将其放入寄存器satp中
        sfence_vma(); // 清空TLB
...
#if !defined (LAB_FS)
    if(found == 0) {
      kvminithart();  // 当目前没有进程运行时，应satp载入全局的内核页表
      intr_on();
      asm volatile("wfi");
    }
#else
